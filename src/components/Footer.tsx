...
export const Footer: React.FC<FooterProps> = ({ onOpenBooking }) => {
  return (
    <footer id="contact" className="relative bg-[#122620] text-white pt-16 sm:pt-20 pb-12 px-4 sm:px-8 rounded-t-[48px] overflow-hidden -mt-6 z-20">
      
      {/* Background soft ambient orbs */}
      <motion.div
        animate={{ scale: [1, 1.15, 1], opacity: [0.15, 0.25, 0.15] }}
        transition={{ repeat: Infinity, duration: 6, ease: 'easeInOut' }}
        className="absolute top-0 right-0 w-96 h-96 bg-[#4BB88E]/20 rounded-full blur-3xl pointer-events-none"
      />
      <motion.div
        animate={{ scale: [1, 1.2, 1], opacity: [0.1, 0.2, 0.1] }}
        transition={{ repeat: Infinity, duration: 7, ease: 'easeInOut', delay: 1 }}
        className="absolute bottom-0 left-0 w-80 h-80 bg-[#F4F2BA]/15 rounded-full blur-3xl pointer-events-none"
      />
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 pb-14 border-b border-white/10">
          <div>
            <PopBadge className="mb-3">
              <span className="text-xs font-black uppercase tracking-widest text-[#122620] bg-[#F4F2BA] px-3.5 py-1.5 rounded-full inline-block font-['Outfit',sans-serif]">
            ...
            >
              ...
              <motion.span
                animate={{
                  textShadow: [
                    '0 0 0px #F4F2BA',
                    '0 0 12px rgba(244,242,186,0.4)',
                    '0 0 0px #F4F2BA'
                  ]
                }}
                transition={{ repeat: Infinity, duration: 3, ease: 'easeInOut' }}
                className="text-[#F4F2BA] underline decoration-[#4BB88E] decoration-wavy decoration-2 inline-block"
              >
                best smile?
              </motion.span>
            </motion.h2>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <motion.button
              ...
              className="bg-[#F4F2BA] hover:bg-[#eae89f] text-[#122620] font-black text-sm sm:text-base px-8 py-4 rounded-full shadow-2xl transition-colors font-['Outfit',sans-serif] flex items-center gap-2 cursor-pointer border-2 border-white"
            >
              <span>BOOK APPOINTMENT</span>
              <ArrowUpRight className="w-5 h-5 text-[#122620]" />
            </motion.button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 py-12 border-b border-white/10 text-slate-300">
          ...
          >
            <div className="flex items-center gap-2.5">
              <motion.div
                ...
                className="w-9 h-9 rounded-full bg-[#F4F2BA] text-[#122620] flex items-center justify-center font-bold shadow-md"
              >
            ...
            <div className="flex items-center gap-2 text-[#F4F2BA] text-xs font-bold pt-1">
              <div className="flex">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className="w-3.5 h-3.5 fill-[#F4F2BA] text-[#F4F2BA]" />
                ))}
              </div>
          ...
          >
            ...
            <motion.a
              ...
              whileHover={{ x: 4 }}
              className="inline-flex items-center gap-1.5 text-xs text-[#F4F2BA] font-bold hover:underline pt-1 cursor-pointer"
            >
          ...
          >
            <span className="text-xs font-black text-white uppercase tracking-wider block font-['Outfit',sans-serif] flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5 text-[#F4F2BA]" />
              Clinic Hours
            </span>
            ...
            <p className="text-xs text-[#4BB88E] pt-1 font-bold">
        ...
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-400 gap-4">
          ...
          >
            Crafted for healthy smiles with <Heart className="w-3.5 h-3.5 text-[#F4F2BA] fill-[#F4F2BA] animate-pulse" />
          </motion.p>
        </div>
